...
from api.grpc_impl import AuditServiceServicer
from http.routes import router, configure_routes

from audit.v1 import audit_pb2_grpc as pb_grpc

logger = get_logger(__name__)


async def serve() -> None:
    setup_logging(service_name="audit-svc", json_format=config.env != "dev")
    ...
    app = FastAPI(
        title="Audit Service",
        version="0.1.0",
        description="Compliance Audit Service — 3-Tier Risk Control",
    )
    ...
    def _make_svc():
        ...
        return AuditService(db=db.session())
    ...
    servicer = AuditServiceServicer(service_factory=_make_svc)
    pb_grpc.add_AuditServiceServicer_to_server(servicer, grpc_server)
