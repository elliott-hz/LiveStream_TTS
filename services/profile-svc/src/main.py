...
from api.grpc_impl import ProfileServiceServicer
...
async def serve() -> None:
    setup_logging(service_name="profile-svc", json_format=config.env != "dev")
    ...
    app = FastAPI(
        ...
        version="0.1.0",
        description="User Profile & Audience Segmentation Service",
    )
    ...
    def _make_svc():
        ...
        return ProfileService(db=db.session())
    ...
    servicer = ProfileServiceServicer(service_factory=_make_svc)
    pb_grpc.add_ProfileServiceServicer_to_server(servicer, grpc_server)
