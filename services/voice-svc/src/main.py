"""
Entrypoint for voice-svc.

Starts both:
  1. gRPC server (port from config, default 50051)
  2. HTTP / health server (port from config, default 8007)

Usage:
    python services/voice-svc/src/main.py
"""
...
from api.grpc_impl import VoiceServiceServicer
...
async def serve() -> None:
    ...
    setup_logging(service_name="voice-svc", json_format=config.env != "dev")
    ...
    app = FastAPI(
        title="Voice Service",
        version="0.1.0",
        description="Digital Human Livestream Platform — Voice/Timbre Management Service",
    )
    ...
    grpc_server = create_grpc_server(
        service_name="voice-svc",
        port=GRPC_PORT,
    )

    async def _make_service() -> "VoiceService":
        from services.voice_service import VoiceService as VS
        return VS(db=db.session())

    servicer = VoiceServiceServicer(voice_service_factory=_make_service)
    pb_grpc.add_VoiceServiceServicer_to_server(servicer, grpc_server)
