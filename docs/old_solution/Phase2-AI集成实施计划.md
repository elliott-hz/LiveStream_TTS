...
8. [风险与应对](#8-风险与应对)
9. [验收标准](#9-验收标准)
10. [部署与配置指南](#10-部署与配置指南) ← 从零开始，一步步操作
...
| `interact-svc/src/config.py` | 新增 `LLM_API_KEY`、`LLM_MODEL=deepseek-v4-flash`、`LLM_API_BASE` |
...
> **你现在的位置：** Phase 2 代码已写完，还没有任何云资源。
> **目标：** 在本机 Mac 上逐个验证每个服务能调通云端 API。
> **原则：** 每一步都给你可以直接复制粘贴的命令。不需要 Docker，不需要 ECS。
...
```bash
# 1. 确认在项目根目录
cd /Users/elliott/vscode_workplace/LiveStream_TTS
pwd   # 必须输出: /Users/elliott/vscode_workplace/LiveStream_TTS

# 2. 确认 Python 版本
python3 --version   # 必须是 3.11.x
```

如果 `pwd` 输出不对，先 `cd /Users/elliott/vscode_workplace/LiveStream_TTS`。

---

### 10.2 ✅ 已申请：阿里云 TTS Key

> 状态：**已完成**。你已拿到以下 3 个值：

| 你手上的东西 | 示例格式 | 代码里的变量名 |
|---|---|---|
| AccessKey ID | `LTAI5t...` | `TTS_ALIYUN_ACCESS_KEY_ID` |
| AccessKey Secret | 长字符串 | `TTS_ALIYUN_ACCESS_KEY_SECRET` |
| AppKey | 数字串 | `TTS_ALIYUN_APP_KEY` |

---

### 10.3 ✅ 已申请：DeepSeek API Key

> 状态：**已完成**。你已拿到以下值：

| 你手上的东西 | 示例格式 | 代码里的变量名 |
|---|---|---|
| API Key | `sk-xxx...` | `DEEPSEEK_API_KEY` |

---

### 10.4 创建 .env 文件（现在做，5 分钟）

把上面拿到的 4 个值填入配置文件，代码才能读到。
...
**2. 用编辑器打开 `.env`，把下面内容复制进去，然后把你手上的真实 Key 替换掉占位符：**
...
# ── 阿里云 TTS（你在 10.2 拿到的 3 个值）──
TTS_BACKEND=cloud
TTS_ALIYUN_ACCESS_KEY_ID=替换成你的AccessKeyID
TTS_ALIYUN_ACCESS_KEY_SECRET=替换成你的AccessKeySecret
TTS_ALIYUN_APP_KEY=替换成你的AppKey
TTS_ALIYUN_ENDPOINT=wss://nls-gateway-cn-shanghai.aliyuncs.com/ws/v1
TTS_CLOUD_VOICE=Aixia
TTS_CLOUD_SPEECH_RATE=0
TTS_CLOUD_VOLUME=80

# ── DeepSeek LLM（你在 10.3 拿到的值）──
DEEPSEEK_API_KEY=替换成你的DeepSeekKey
...
# ── Render（不改，用默认值）──
RENDER_ENGINE=viseme
```

**3. 替换完成后，验证文件内容（以阿里云为例，确认不是占位符）：**

```bash
cat .env | grep TTS_ALIYUN_ACCESS_KEY_ID
# 必须输出: TTS_ALIYUN_ACCESS_KEY_ID=LTAI5t...  （以 LTAI 开头，不是"替换成"）
```

> **注意：** `.env` 已在 `.gitignore` 中，不会被 git 提交。永远不要把包含真实 Key 的 `.env` 上传到 GitHub。

---

### 10.5 安装 Python 依赖（一次性，2 分钟）

```bash
cd /Users/elliott/vscode_workplace/LiveStream_TTS

# 安装所有服务共用的依赖
pip install httpx pypinyin grpcio grpcio-tools protobuf

# 安装各服务自身依赖
pip install -e libs/common -e libs/db -e libs/kafka --no-deps
```

---

### 10.6 验证 tts-svc（最关键的一步）

> 验证阿里云 TTS API 能否正常合成语音。
...
python services/tts-svc/src/main.py
```

**期望看到：**
```
[tts-svc] tts_svc.starting version=0.2.0
[tts-svc] tts_svc.cloud_backend voice=Aixia endpoint=wss://...
[tts-svc] grpc.server.started port=50059
[tts-svc] tts_svc.ready grpc_port=50059 http_port=8008
```

**如果看到 `cloud_backend voice=Aixia`，说明配置读取成功。服务保持运行，不要关掉这个终端。**
...
# 用 Python 脚本调 gRPC 验证
python3 -c "
import sys
sys.path.insert(0, '.')
sys.path.insert(0, 'libs/proto')

import grpc
from tts.v1 import tts_pb2, tts_pb2_grpc
...
print('→ 正在调用阿里云 TTS...')
try:
    for i, resp in enumerate(stub.Synthesize(request)):
        size = len(resp.audio_chunk)
        print(f'  收到第 {i+1} 个音频包: {size} bytes')
    print('✅ TTS 合成成功！')
...
```
→ 正在调用阿里云 TTS...
  收到第 1 个音频包: 1280 bytes
  收到第 2 个音频包: 1280 bytes
  ...
✅ TTS 合成成功！
```
...
| 错误现象 | 原因 | 解决方法 |
|---|---|---|
| `cloud_backend` 没出现在日志中 | `TTS_BACKEND` 没加载 | `echo $TTS_BACKEND` 确认是 `cloud` |
| `StatusCode.UNAUTHENTICATED` | AccessKey 错误 | 重新核对 `.env` 里的 `TTS_ALIYUN_ACCESS_KEY_ID` 和 `TTS_ALIYUN_ACCESS_KEY_SECRET` |
| `connection refused` | 服务没启动 | 回到 10.6.1 启动服务 |
| `import error: No module named 'tts'` | proto 路径不对 | 确认命令中有 `sys.path.insert(0, 'libs/proto')` |

---

### 10.7 验证 nlp-svc

> 验证 NLP 分析（意图识别、情感分析、敏感词检测）。
...
cd /Users/elliott/vscode_workplace/LiveStream_TTS
set -a && source .env && set +a
pip install pypinyin 2>/dev/null   # 确保拼音库已装
python services/nlp-svc/src/main.py
```

**期望看到：**
```
[nlp-svc] nlp-svc.main.starting http=0.0.0.0:8009 grpc=:50060
```
...
import grpc
from nlp.v1 import nlp_pb2, nlp_pb2_grpc

channel = grpc.insecure_channel('localhost:50060')
stub = nlp_pb2_grpc.NLPServiceStub(channel)

resp = stub.AnalyzeDanmaku(nlp_pb2.AnalyzeDanmakuRequest(
    text='这个口红多少钱',
    live_room_id='test',
    user_name='test_user'
))
print(f'意图: {resp.intent}')
print(f'情感: {resp.sentiment}')
print(f'是否敏感: {resp.is_sensitive}')
print('✅ NLP 分析成功！')
"
```

**期望结果：** 输出意图、情感、敏感词检测结果，`is_sensitive=false`。

---

### 10.8 验证 interact-svc（DeepSeek LLM）

> 验证 DeepSeek API 能否根据弹幕生成回复。
...
cd /Users/elliott/vscode_workplace/LiveStream_TTS
set -a && source .env && set +a
python services/interact-svc/src/main.py
```

**期望看到：**
```
[interact-svc] interact.started
```

> ⚠️ 日志中可能出现 `kafka.connect_failed` 警告 — **忽略它**，本地没有 Kafka 不影响 LLM 调用的验证。
...
import grpc
from interact.v1 import interact_pb2, interact_pb2_grpc

channel = grpc.insecure_channel('localhost:50062')
stub = interact_pb2_grpc.InteractionServiceStub(channel)

resp = stub.ProcessDanmaku(interact_pb2.ProcessDanmakuRequest(
    danmaku=interact_pb2.DanmakuMessage(
        text='这个口红多少钱',
        user_name='test_user'
    ),
    live_room_id='test_room',
    session_id='test_session'
))
print(f'是否回复: {resp.should_reply}')
print(f'回复内容: {resp.reply_text}')
print(f'情感标签: {resp.emotion_tag}')
print('✅ interact-svc 调用成功！')
"
```

**期望结果：**
- 有 `DeepSeek API Key`：返回 LLM 生成的回复文本
- 没有 Key：自动降级，返回关键词模板回复（同样可用）
- 两种情况都算成功 ✅

---

### 10.9 验证 knowledge-svc + render-svc
...
resp = stub.Search(knowledge_pb2.SearchRequest(
    query='口红价格',
    kb_id='default',
    top_k=3
))
print(f'找到 {len(resp.results)} 条结果')
print('✅ knowledge-svc 调用成功！')
"
```
...
cd /Users/elliott/vscode_workplace/LiveStream_TTS
set -a && source .env && set +a
python services/render-svc/src/main.py
```
...
import grpc
from render.v1 import render_pb2, render_pb2_grpc

channel = grpc.insecure_channel('localhost:50061')
stub = render_pb2_grpc.RenderServiceStub(channel)

# 用一段静音 PCM 数据测试 Viseme 引擎
resp = stub.RenderFrame(render_pb2.RenderFrameRequest(
    audio_data=b'\x00\x00' * 640,   # 40ms @ 16kHz
    avatar_id='default',
    frame_width=720,
    frame_height=720
))
print(f'帧大小: {len(resp.frame_data)} bytes')
print('✅ render-svc 调用成功！')
"
```

---

### 10.10 下一步：Docker 安装（需要全链路联调时再装）

> 上面 10.6 — 10.9 的验证**不需要 Docker**。只有当你需要全链路联调（多个服务同时启动 + Kafka 消息流转）时，才需要 Docker。

**安装 Docker Desktop（Mac）：**
...
# 安装完成后：
# 1. 在 Applications 找到 Docker 图标，双击启动
# 2. 等待菜单栏出现 Docker 鲸鱼图标
# 3. 回到终端验证：
docker --version
...
docker compose --profile phase2 up -d milvus   # Milvus 向量库

# 确认都在运行
docker compose ps
# 所有容器 STATUS 应该显示 healthy
```

> **不需要买 ECS。** 本地 Mac 就能完成全部调试。ECS 是准备上线面向真实用户时才需要买。

---

### 10.11（可选）K8s ConfigMap 更新清单

> 当你准备好 ECS 服务器后，需要更新以下 K8s 配置文件。

| 文件 | 新增内容 |
|---|---|
| `services/tts-svc/k8s/configmap.yaml` | `TTS_BACKEND: cloud`、`TTS_ALIYUN_ENDPOINT`、`TTS_CLOUD_VOICE`、`TTS_CLOUD_SPEECH_RATE`、`TTS_CLOUD_VOLUME` |
| `services/tts-svc/k8s/secret.yaml`（新建） | `TTS_ALIYUN_ACCESS_KEY_ID`、`TTS_ALIYUN_ACCESS_KEY_SECRET`、`TTS_ALIYUN_APP_KEY` |
| `services/interact-svc/k8s/configmap.yaml` | `LLM_MODEL`、`LLM_MAX_TOKENS`、`LLM_TEMPERATURE`、`LLM_ENABLE_FALLBACK`、`DEEPSEEK_BASE_URL` |
| `services/interact-svc/k8s/secret.yaml`（新建） | `DEEPSEEK_API_KEY` |
| `services/nlp-svc/k8s/configmap.yaml` | `NLP_BACKEND`、`PINYIN_VARIANT_ENABLED` |
| `services/knowledge-svc/k8s/configmap.yaml` | `VECTOR_BACKEND`、`EMBEDDING_BACKEND`、`EMBEDDING_MODEL`、`MILVUS_HOST`、`MILVUS_PORT` |
| `services/render-svc/k8s/configmap.yaml` | `RENDER_ENGINE: viseme`、`AVATAR_IMAGE_DIR` |
...
Secret（新建 `services/tts-svc/k8s/secret.yaml`）：
```yaml
apiVersion: v1
kind: Secret
metadata:
  name: tts-svc-secrets
  namespace: livestream-tts
type: Opaque
stringData:
  TTS_ALIYUN_ACCESS_KEY_ID: "你的真实AK"
  TTS_ALIYUN_ACCESS_KEY_SECRET: "你的真实SK"
  TTS_ALIYUN_APP_KEY: "你的真实AppKey"
```

> ⚠️ Secret 文件不要提交到 Git。
